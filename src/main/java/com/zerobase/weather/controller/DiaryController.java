...
public class DiaryController {
    ...
    @ApiOperation(value = "일기 택스트와 날씨를 이용해서 DB에 일기 저장", notes = "이것은 노트")
    @PostMapping("/create/diary")
    void createDiary(
            @RequestParam
            @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
            @ApiParam(value = "추가할 일기의 날짜", example = "2020-02-02")
            LocalDate date,
    ...
    List<Diary> readDiary(
            @RequestParam
            @DateTimeFormat(iso = DateTimeFormat.ISO.DATE)
            @ApiParam(value = "조회할 일기의 날짜", example = "2020-02-02")
            LocalDate date
    ){
    ...
    @ApiOperation("선택한 닐찌의 일기 데이터를 수정 합니다.")
    ...
    @ApiOperation("선택한 닐찌의 일기 데이터를 삭제 합니다.")
